...
async function startQuiz() {
  ...
  try {
    // Use Drive notes if loaded
    const notesContext = quizDriveNotes ? `\n\nHere are Owen's actual class notes to base questions on:\n${quizDriveNotes.substring(0, 8000)}` : '';
    const questionsJson = await generateQuizQuestions(className, topic, count, notesContext);
...
async function generateQuizQuestions(className, topic, count, notesContext = ) {
